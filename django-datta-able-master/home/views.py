...
from pipeline.crawler import create_index, mappings
...
def index(request):
  ...
  results = es.search(index='news_*', body={'from':0, 'size':10, 'query':{'match_all':{}}})
  results_today = es.search(index=f'news_{ymd}', body={'from':0, 'size':10, 'query':{'match_all':{}}})
  recent_news = es.search(index='news_*', body={'from':0, 'size':10, 'query':{'match_all':{}},"sort": [
    {
      "timestamp": {
        "order": "desc"
      }
  ...
  context = {
    ...
    'today_total': results_today.get('hits').get('total').get('value'),
    'recent' : [Product(**i.get('_source')) for i in recent_news.get('hits').get('hits')],
    'sync': DefaultDate.objects.get_or_create(date = ymd_format)[0].sync,
    'crawl': DefaultDate.objects.get_or_create(date = ymd_format)[0].crawl,
  }
